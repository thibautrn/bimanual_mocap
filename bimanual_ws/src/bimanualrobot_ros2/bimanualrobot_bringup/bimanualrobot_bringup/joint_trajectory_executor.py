...
class JointTrajectoryExecutor(Node):
    ...
    def __init__(self):
        ...
        # Get which arm to use ('left' or 'right'), default to 'left'
        use_arm = self.declare_parameter('use_arm', 'left').get_parameter_value().string_value

        # Select parameter namespace based on arm
        if use_arm == 'right':
            param_ns = 'right_joint_trajectory_executor'
        else:
            param_ns = 'left_joint_trajectory_executor'
        ...
        self.declare_parameter(
            'action_topic', f'/{use_arm}_arm_controller/follow_joint_trajectory'
        )
