...
LOG_PATH = "/home/asurite.ad.asu.edu/troisin/Documents/robot/bimanual_mocap/bimanual_ws/src/bimanualrobot_ros2/bimanualrobot_system_tests/scripts/logs/bimanual_robot_log.txt"
...
PER_MOVE_SECONDS    = 1.5    # big time first; we can crank down later
...
# ---- tunables ----
W_ORI = 0.2          # tune 0.05–1.0
ALLOW_FLIP = False   # True if +Z and -Z are both okay

def residual_pos_plus_wrist3_axis(q_vars, model, data,
                                  fid_wrist_pos, fid_axis_frame,
                                  idx_q_vars, target_W,
                                  w_pos=W_POS, w_ori=W_ORI, w_reg=W_REG,
    ...
    # --- position term (unchanged) ---
    pW = data.oMf[fid_wrist_pos].translation
    err_pos = (pW - target_W) * w_pos   # shape (3,)

    # --- wrist_3 axis alignment: local X -> world Z+ ---
    R_axis = data.oMf[fid_axis_frame].rotation     # 3x3
    nx = R_axis[:, 0]                              # wrist_3 local X in world
    zhat = np.array([0.0, 0.0, 1.0])
    if allow_flip and np.dot(nx, zhat) < 0.0:
        zhat = -zhat
    # cross-product error is zero when nx ∥ zhat
    err_axis = np.cross(nx, zhat) * w_ori          # shape (3,)

    # --- tiny regularization on the IK variables ---
    err_reg = w_reg * q_vars                       # shape (ndof,)

    return np.hstack((err_pos, err_axis, err_reg))
...
def solve_wrist_ik_least_squares(robot: RobotWrapper,
    ...
    res = least_squares(
        residual_pos_plus_wrist3_axis,
        seed,
        bounds=(lb, ub),
        args=(model, data, fid_wrist, fid_axis, idx_q_vars, target_W, W_POS, W_ORI, W_REG),
        max_nfev=MAX_ITERS,
...
class PinIKUDPToFJT(Node):
    def __init__(self):
        ...
        self.fid_axis = self.model.getFrameId(F_AXIS)
    ...
    def run_udp_follow(self):
        ...
        # your three snapshots
        uarm1, larm1, hand1 = (-0.16791768372058868, 0.43098410964012146, -0.030094459652900696), (-0.4220631420612335, 0.4452546536922455, -0.0830702930688858), (-0.641180157661438, 0.43876174092292786, -0.10165978223085403)
        uarm2, larm2, hand2 = (-0.16298747062683105, 0.43098410964012146, -0.050369761884212494), (-0.19722092151641846, 0.41626572608947754, 0.20694607496261597), (0.022760596126317978, 0.41402554512023926, 0.20871058106422424)
        uarm3, larm3, hand3 = (-0.16856712102890015, 0.43098410964012146, 0.02621355466544628), (-0.15296421945095062, 0.4011036157608032, 0.2840191125869751), (-0.06981884688138962, 0.34103548526763916, 0.478643536567688)
        uarm4, larm4, hand4 = (-0.17056234180927277, 0.43098410964012146, -0.0032423182856291533), (-0.26740750670433044, 0.18990403413772583, -0.013312861323356628), (-0.3111858069896698, -0.017011160030961037, 0.04726405814290047)

        samples = [(uarm1, larm1, hand1), (uarm2, larm2, hand2), (uarm3, larm3, hand3),(uarm3, larm3, hand3), (uarm4, larm4, hand4), (uarm4, larm4, hand4)]

        for idx, (uarm, larm, hand) in enumerate(samples, 1):
            if not rclpy.ok():
                break

            # 1) scale wearable -> robot targets
            _, W = scale_watch_to_robot(uarm, larm, hand)
            ...
            # 3) solve wrist-only IK
            startime = time.time()
            ok, qvars, qfull, pW, err, iters = solve_wrist_ik_least_squares(
                self.robot, self.fid_wrist, self.fid_axis,  # keep your signature
                self.idx_q_vars, self.lb, self.ub, W, seed=seed
            )
            print(f"[{idx}] solve time: {time.time() - startime:.4f}s")
            ...
            if not ok:
                print("[IK] did not converge; NOT sending.")
                time.sleep(2.0)
                continue
