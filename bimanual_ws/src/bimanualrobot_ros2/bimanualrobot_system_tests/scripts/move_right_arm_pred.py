...
RH_ENABLE       = True
RH_HORIZON_S    = 0.5      # total horizon (seconds)
RH_N_STEPS      = 10         # number of waypoints in the horizon
RH_MIN_DT       = 0.06     # safety
VEL_EMA_ALPHA   = 0.7       # EMA on wrist velocity (0…1)
...
# Cycle & timing
CYCLE_SECONDS       = 0.05   # how often we compute IK / (optionally) send
PER_MOVE_SECONDS    = 0.1    # big time first; we can crank down later
...
class LowPassEMA:
    ...
    def _alpha(self, dt):
        # bilinear-style mapping from cutoff to smoothing factor
    ...
    def update(self, x, dt):
        ...
        if self.y is None:  # init with first sample
            self.y = x.copy()
            return self.y
        a = self._alpha(dt)
...
class PinIKUDPToFJT(Node):
    def __init__(self):
        ...
        self._w_hist = deque()    # (timestamp, np.array([x,y,z]))
        self._w_vel  = np.zeros(3)  # EMA velocity estimate in m/s

        # self._w_lpf = LowPassEMA(dim=3, fc_hz=3.0)   # 2–5 Hz is a good start
        # Optional: filter joint commands too
    ...
    def _rh_predict_wrist_traj(self, W_now):
        """Return list of RH_N_STEPS wrist targets over RH_HORIZON_S with constant-vel prediction."""
    ...
    # --- RHC: solve IK along short horizon, warm-starting each step ---
    def _rh_solve_horizon(self, W_list, seed_qvars, js_now):
        """
        W_list: list of wrist targets in base frame
        seed_qvars: last qvars (len = len(IK_JOINTS))
        js_now: current measured full JOINTS positions (for filling non-IK joints)
        Returns: list of full q_cmd points (len <= len(W_list)), dt_step
        """
        if not W_list:
            return [], 0.0
        ...
        for Wi in W_list:
            ...
            # build a full command for this waypoint (IK joints from qvars, others hold current)
            q_cmd = list(js_now)
            for jn, val in zip(IK_JOINTS, qvars):
                if jn in JOINTS:
                    q_cmd[JOINTS.index(jn)] = float(val)

            # optional safety: check validity, truncate horizon if invalid
            if not self._is_state_valid(q_cmd):
                break

            q_points.append(q_cmd)
            qvars_seed = qvars  # warm-start next step
            last_good = q_cmd
    ...
    def run_udp_follow(self):
        ...
        while rclpy.ok():
            ...
            # W = self._w_lpf.update(W, dt)  # filtered wrist target BEFORE RHC/IK


            # === RHC: update history & predict wrist horizon ===
            ...
            # 3) current measured joints (to fill non-IK joints)
            js_now = self._current_positions(JOINTS, default=0.0)

            if RH_ENABLE:
                # ---- Receding-horizon solve (sequential IK along short horizon) ----
                q_points, dt_step = self._rh_solve_horizon(W_list, seed, js_now)

                # log the first point for debugging & keep last IK seed
                if q_points:
                    first_q = q_points[0]
                    self._last_qvars = [first_q[JOINTS.index(jn)] for jn in IK_JOINTS]
                    # optional rate limit on the *first* step
                    q_points[0] = self._limit_step(q_points[0])

                    # send short multi-point traj (preempts previous goal)
                    if SEND_TO_CONTROLLER:
                        self._send_followtraj_traj(q_points, dt_step)

                    # pretty print snapshot
                    print("\n===== RH IK cycle =====")
                    print(f"W_now: {np.round(W,6)}  horizon_pts: {len(q_points)}  dt_step: {dt_step:.3f}s")
                    print("First command (rad):", [round(v,6) for v in q_points[0]])
                else:
                    print("[RHC] No valid horizon points; skipping send.")
                    continue

            else:
                # ---- original single-step behavior (kept as fallback) ----
                ok, qvars, qfull, pW, err, iters = solve_wrist_ik_least_squares(
                    self.robot, self.fid_wrist, self.idx_q_vars, self.lb, self.ub, W, seed=seed
                )

                print("\n===== IK cycle (single-step) =====")
                ...
                # RHC: filter only the first point (the one executed soonest)
                q_points[0] = self._q_lpf.update(q_points[0], dt)
