...
# Cycle & timing
CYCLE_SECONDS       = 0.1   # how often we compute IK / (optionally) send
PER_MOVE_SECONDS    = 1    # big time first; we can crank down later
...
CSV_PATH = "/home/thibaut/Documents/Bimanual_Robot/logs/udp_samples.csv"
import csv
def udp_listener(port=UDP_PORT):
    ...
    with open(CSV_PATH, "r", newline="") as f:
        ...
        for row in r:
            with _udp_lock:
                _hand_pos = (float(row["hand_px"]), float(row["hand_py"]), float(row["hand_pz"]))
                _larm_pos = (float(row["larm_px"]), float(row["larm_py"]), float(row["larm_pz"]))
                _uarm_pos = (float(row["uarm_px"]), float(row["uarm_py"]), float(row["uarm_pz"]))
            # optional: pace by recorded timestamps
            ts = float(row["ts"])
