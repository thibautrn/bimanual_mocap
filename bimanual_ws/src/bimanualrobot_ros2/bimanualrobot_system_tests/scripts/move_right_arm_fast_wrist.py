...
W_ORI_YAW = 0.5     # weight for "forward points +X" in-plane yaw
W_ORI_Z   = 1.0 
ALLOW_FLIP = False

def residual_wrist_only(q_vars, model, data, fid_wrist, fid_axis_frame, idx_q_vars, target_W, w_pos=W_POS, w_reg=W_REG):
...
def residual_wrist_only_point(q_vars, model, data,
                                    fid_wrist, fid_axis_frame, idx_q_vars,
                                    target_W, w_pos=W_POS, w_reg=W_REG,
                                    w_ori_z=W_ORI_Z, w_ori_yaw=W_ORI_YAW):
    """
    Pos + orientation residual:
      - wrist position -> target_W            (3)
      - 'palm horizontal': local X ‖ ±world Z (3 via cross)
      - 'face +X': local Y projected to XY faces +X (1 scalar yaw error)
      - tiny L2 reg on IK vars                (ndof)
    """
    # build full q and forward kinematics
    q = full_q_from_vars(model, idx_q_vars, q_vars)
    pin.forwardKinematics(model, data, q)
    pin.updateFramePlacements(model, data)
    ...
    # orientation frame (assumes columns are local X,Y,Z expressed in world)
    R_axis = data.oMf[fid_axis_frame].rotation
    nx = R_axis[:, 0]   # local X  (we treat this as the palm normal)
    ny = R_axis[:, 1]   # local Y  (we treat this as the 'forward' direction)

    zhat = np.array([0.0, 0.0, 1.0])
    xhat = np.array([0.0, 1.0, 0.0])

    # --- keep palm horizontal: local X parallel to world ±Z
    # (cross-product residual goes to zero when vectors are parallel)
    if ALLOW_FLIP and np.dot(nx, zhat) < 0.0:
        z_use = -zhat
    else:
        z_use = zhat
    err_horiz = np.cross(nx, z_use) * w_ori_z   # shape (3,)

    # --- fix yaw so the palm 'faces' +X in the XY plane using local Y
    # project ny onto the XY plane, normalize, and measure yaw error vs +X
    ny_xy = ny - np.dot(ny, zhat) * zhat
    n = np.linalg.norm(ny_xy)
    if n > 1e-9:
        ny_xy /= n
        # scalar signed-yaw error around Z: + if ny_xy ccw from +X to ny_xy
        # (this is z-component of cross(ny_xy, xhat))
        yaw_err_scalar = np.dot(np.cross(ny_xy, xhat), zhat)
    else:
        # if projection is degenerate, don't penalize yaw this tick
        yaw_err_scalar = 0.0
    err_yaw = np.array([w_ori_yaw * yaw_err_scalar])  # shape (1,)

    # regularization on IK variables
    err_reg = w_reg * q_vars

    # stack residual
    return np.hstack((err_pos, err_horiz, err_yaw, err_reg))
...
def solve_wrist_ik_least_squares(robot: RobotWrapper,
    ...
    res = least_squares(
        ...
        bounds=(lb, ub),
        args=(model, data, fid_wrist, fid_axis, idx_q_vars, target_W, W_POS, W_REG),
        ...
        while rclpy.ok():
            ...
            for Wk in W_list:
                ok, qvars, qfull, pW, err = solve_wrist_ik_least_squares(
                    self.robot, self.fid_wrist, self.fid_axis, self.idx_q_vars, self.lb, self.ub, Wk, seed=qvars_seed
                )
                if not ok:
                    break
                q_cmd = list(js_now)
