...
class PoseEstPublisherUDP:
    ...
    def publish_loop(self, q: queue):
        self.__active = True
        while self.__active:
            # print(f"HELLO HELLO HGELLO HELLO HELLO HELLO HGELLO HELLO HELLO HELLO HGELLO HELLO HELLO HELLO HGELLO HELLO HELLO HELLO HGELLO HELLO")  

            try:

                # Get the latest IMU data from the queue
                msg = q.get(timeout=2)
                while q.qsize() > 5:
                    msg = q.get(timeout=2)
            except queue.Empty:
            ...
            # Print the raw data before packing

            # Pack the message into binary format for UDP transmission
